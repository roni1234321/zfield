...
function terminalApp() {
    return {
        ...
        windowResizeStartWidth: 0,
        windowResizeStartHeight: 0,
        windowResizeStartScreenX: 0,
        windowResizeStartScreenY: 0,
        isWindowDragging: false,
        windowDragStartX: 0,
        windowDragStartY: 0,
        windowDragStartPosX: 0,
        windowDragStartPosY: 0,
        ...
        // Window control functions (using direct pywebview API)
        isMaximized: false,
        windowMinimize() {
            if (window.pywebview && window.pywebview.api) {
                window.pywebview.api.minimize();
            } else {
                // Fallback to HTTP API if direct API not available
        ...
        windowMaximize() {
            if (window.pywebview && window.pywebview.api) {
                if (this.isMaximized) {
                    ...
                    if (window.pywebview.api.restore) {
                        window.pywebview.api.restore();
                    } else {
                        // Fallback: try maximize again (should toggle)
                ...
                fetch(`/api/window/${action}`, { method: 'POST' })
                    ...
                    .catch(err => console.error(`Failed to ${action} window:`, err));
            }
        },
        windowClose() {
            if (window.pywebview && window.pywebview.api) {
        ...
        startWindowResize(direction, e) {
            ...
            this.windowResizeDirection = direction;

            // Use screen coordinates for accurate tracking
            this.windowResizeStartX = e.screenX;
            this.windowResizeStartY = e.screenY;

            // Get current window size and position
            // In pywebview, we need to get these from the API if available
            if (window.pywebview && window.pywebview.api && window.pywebview.api.getWindowSize) {
                const size = window.pywebview.api.getWindowSize();
                this.windowResizeStartWidth = size.width;
                this.windowResizeStartHeight = size.height;
            } else {
                this.windowResizeStartWidth = window.outerWidth || window.innerWidth;
                this.windowResizeStartHeight = window.outerHeight || window.innerHeight;
            }

            if (window.pywebview && window.pywebview.api && window.pywebview.api.getWindowPosition) {
                const pos = window.pywebview.api.getWindowPosition();
                this.windowResizeStartScreenX = pos.x;
                this.windowResizeStartScreenY = pos.y;
            } else {
                this.windowResizeStartScreenX = window.screenX || window.screenLeft || 0;
                this.windowResizeStartScreenY = window.screenY || window.screenTop || 0;
            }
            ...
            // Prevent window dragging during resize
        ...
        doWindowResize(e) {
            if (!this.isWindowResizing || !this.windowResizeDirection) return;
            if (!e) return;

            // Use screen coordinates for accurate resizing
            const currentScreenX = e.screenX;
            const currentScreenY = e.screenY;

            const deltaX = currentScreenX - this.windowResizeStartX;
            const deltaY = currentScreenY - this.windowResizeStartY;

            let newWidth = this.windowResizeStartWidth;
            let newHeight = this.windowResizeStartHeight;
            let newX = this.windowResizeStartScreenX;
            let newY = this.windowResizeStartScreenY;

            const direction = this.windowResizeDirection;
            const minWidth = 400;
            const minHeight = 300;

            if (direction.includes('right')) {
                newWidth = Math.max(minWidth, this.windowResizeStartWidth + deltaX);
            }
            if (direction.includes('left')) {
                const widthChange = this.windowResizeStartWidth - deltaX;
                if (widthChange >= minWidth) {
                    newWidth = widthChange;
                    newX = this.windowResizeStartScreenX + deltaX;
                }
            }
            if (direction.includes('bottom')) {
                newHeight = Math.max(minHeight, this.windowResizeStartHeight + deltaY);
            }
            if (direction.includes('top')) {
                const heightChange = this.windowResizeStartHeight - deltaY;
                if (heightChange >= minHeight) {
                    newHeight = heightChange;
                    newY = this.windowResizeStartScreenY + deltaY;
                }
            }

            // Try pywebview API first (most reliable)
            if (window.pywebview && window.pywebview.api && window.pywebview.api.resizeWindow) {
                try {
                    window.pywebview.api.resizeWindow(newWidth, newHeight, newX, newY);
                } catch (err) {
                ...
                fetch('/api/window/resize', {
                    ...
                    body: JSON.stringify({ width: newWidth, height: newHeight, x: newX, y: newY })
        ...
        stopWindowResizing() {
            ...
            this.windowResizeDirection = null;
            document.body.style.cursor = '';
            document.body.style.userSelect = '';
            // Restore drag region for header
            const header = document.querySelector('header');
            if (header) {
        ...
        checkWindowSnap() {
            // Check if window should snap after drag ends
            ...
            let currentX = window.screenX || window.screenLeft || 0;
            let currentY = window.screenY || window.screenTop || 0;
            ...
            // Check for snapping zones
            const snapLeft = currentX <= this.snapThreshold;
            const snapRight = currentX + windowWidth >= screenWidth - this.snapThreshold;
            const snapTop = currentY <= this.snapThreshold;
            const snapBottom = currentY + windowHeight >= screenHeight - this.snapThreshold;
            ...
            if (snapTop && snapLeft) {
                ...
                snapX = 0;
                snapY = 0;
                snapWidth = screenWidth / 2;
                snapHeight = screenHeight / 2;
            } else if (snapTop && snapRight) {
                // Top-right corner: quarter screen
                snapX = screenWidth / 2;
                snapY = 0;
                snapWidth = screenWidth / 2;
                snapHeight = screenHeight / 2;
            } else if (snapBottom && snapLeft) {
                // Bottom-left corner: quarter screen
                snapX = 0;
                snapY = screenHeight / 2;
                snapWidth = screenWidth / 2;
                snapHeight = screenHeight / 2;
            } else if (snapBottom && snapRight) {
                ...
                // Left edge: half screen width, full height
                snapX = 0;
                snapY = 0;
                snapWidth = screenWidth / 2;
                snapHeight = screenHeight;
            } else if (snapRight) {
                // Right edge: half screen width, full height
                snapX = screenWidth / 2;
                snapY = 0;
                snapWidth = screenWidth / 2;
                snapHeight = screenHeight;
            ...
            if (snapX !== null && snapY !== null && snapWidth !== null && snapHeight !== null) {
                if (window.pywebview && window.pywebview.api && window.pywebview.api.resizeWindow) {
                    window.pywebview.api.resizeWindow(snapWidth, snapHeight, snapX, snapY);
                } else {
                    fetch('/api/window/resize', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ width: snapWidth, height: snapHeight, x: snapX, y: snapY })
                    }).catch(err => console.error('Failed to snap window:', err));
                }
                return true;
            }
            return false;
        ...
        startWindowDrag(e) {
            // Track when drag might start (for snapping detection)
            if (e.target.closest('header') && !e.target.closest('button') && !e.target.closest('[style*="no-drag"]')) {
                this.isWindowDragging = true;
            }
        },

        handleWindowDrag(e) {
            // This won't fire during native drag, but we'll check on mouseup
        },

        stopWindowDrag() {
            if (this.isWindowDragging) {
                // Small delay to let window position update after native drag ends
                setTimeout(() => {
                    this.checkWindowSnap();
